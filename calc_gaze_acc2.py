...
def extract_availData_from_GT_and_result(inputPath_GT):
    ...
    df_extGT = extGT[[
                         'CAN_S_Gaze_ROI', 'gt_s_gaze_roi_das', 'gt_name_gaze_roi', 'roi_idx_h','roi_name_h',
                        'max_target_id',	'max_roi_score', 'Load_file'
                    ]]
...
if __name__ == '__main__':
    ...
    inputfile = "./save_output023.csv"
    df_data = extract_availData_from_GT_and_result(inputfile)
    df_data = df_data.astype({'max_roi_score': "int64", "gt_s_gaze_roi_das":"int64"})
    df_data2 = policy_gaze_roi_accuracy(df_data, 'max_roi_score') #max_target_id

    counting_gaze_roi(df_data2)
    save_csvfile(df_data2, 'save2_output.csv')
    print(1/0)
